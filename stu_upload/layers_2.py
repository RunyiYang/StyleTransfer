...
class ConvolutionalLayer(object):
                ...
                for idxh in range(height_out):
                    for idxw in range(width_out):
                        ...
                        self.output[idxn, idxc, idxh, idxw] = np.sum(self.weight[:, :, :, idxc] * \
                                                                     self.input_pad[idxn, :, hs:hs + self.kernel_size,
                                                                     ws:ws + self.kernel_size]) + \
                                                              self.bias[idxc]
    ...
    def forward_speedup(self, input):
        ...
        self.col_image = im2col(self.input_pad, self.kernel_size,
                                self.stride)

        # 4.matrix multiply
        self.output = np.dot(self.col_image, col_weight) + self.bias
        self.output = np.reshape(self.output, np.hstack(([N], [height_out], [width_out], [
            cout])))
    ...
    def backward_speedup(self, top_diff):
        ...
        pad_diff[:, :, self.padding:self.padding + top_diff.shape[2],
        ...
        bottom_diff = np.reshape(bottom_diff, [N, self.input.shape[2], self.input.shape[3],
                                               self.input.shape[1]])  # n*w*w*c -> n,h,w,c
    ...
    def backward_raw(self, top_diff):
            ...
            for idxc in range(top_diff.shape[1]):
                for idxh in range(top_diff.shape[2]):
                    for idxw in range(top_diff.shape[3]):
                        ...
                        hs = idxh * self.stride
                        ws = idxw * self.stride
                        self.d_weight[:, :, :, idxc] += np.dot(top_diff[idxn, idxc, idxh, idxw],
                                                               self.input_pad[idxn, :, hs:hs + self.kernel_size,
                                                               ws:ws + self.kernel_size])
        ...
        bottom_diff = bottom_diff[:, :, self.padding:bottom_diff.shape[2] - self.padding,
                      self.padding:bottom_diff.shape[3] - self.padding]
        self.backward_time = time.time() - start_time
        return bottom_diff
